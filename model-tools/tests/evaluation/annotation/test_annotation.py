...
from model_tools.evaluation.annotation import runtime, stage
from model_tools.evaluation.annotation.detector import (
    MAX_DETECTIONS,
    build_annotation,
    convert_normalized_boxes_to_pixel_xyxy,
    rank_target_aligned_proposals,
)
from model_tools.evaluation.annotation.models import (
    AnnotationRecord,
    AnnotationRequest,
    CudaProtocol,
    DetectorRuntime,
...
def test_load_runtime_selects_cuda_and_moves_model_to_it(
    ...
    monkeypatch.setattr(
        "model_tools.evaluation.annotation.runtime.importlib.import_module",
        fake_import_module,
    )

    runtime_value = runtime.load_runtime(EvaluationPaths(root=tmp_path))

    assert runtime_value.device == "cuda"
    assert runtime_value.model is fake_model
...
def test_load_runtime_falls_back_to_cpu_without_cuda(
    ...
    assert runtime_value.device == "cpu"
    assert runtime_value.model is fake_model
...
def test_annotate_emits_sanitized_lifecycle_and_cadenced_progress(
    ...
    class FakeTorch:
        cuda: CudaProtocol = FakeCuda()

        @staticmethod
        def no_grad() -> object:
            return runtime.NullContext()
    ...
    class FakeTokenizer:
        def __call__(
            ...
            *,
            ...
            _ = text, return_offsets_mapping, add_special_tokens
            return {}

    class FakeProcessor:
        tokenizer: TokenizerProtocol = FakeTokenizer()
    ...
    def fake_load_runtime(_paths: EvaluationPaths) -> DetectorRuntime:
        torch: TorchProtocol = FakeTorch()
        processor: ProcessorProtocol = FakeProcessor()
        model: ModelProtocol = FakeModel()
    ...
    monkeypatch.setattr(stage, "load_runtime", fake_load_runtime)

    def fake_import_module(_name: str) -> type[FakeImageModule]:
        return FakeImageModule

    monkeypatch.setattr(
        "model_tools.evaluation.annotation.stage.importlib.import_module",
        fake_import_module,
    )
    ...
    monkeypatch.setattr(stage, "build_annotation", fake_build_annotation)

    def fake_write_annotation(_destination: Path, _model: object) -> None:
        return None

    monkeypatch.setattr(stage, "write_annotation", fake_write_annotation)

    execution = stage.run(AnnotationRequest(paths=paths))
    summary = execution.summary

    stderr = capsys.readouterr().err
    events = [_parse_json_object(line) for line in stderr.splitlines() if line]
